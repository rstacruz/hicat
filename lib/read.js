...
/**
 * files() : read.files(files, fn)
 * Reads from files. If no files are given, read from stdin.
 *
 *   fnames = ['readme.txt']
 *   read.files(fnames, function (err, data) {
...
exports.files = function (files, fn) {
  // from stdin
  if (!files || files.length === 0) {
    exports.stdin(function (err, data) {
      if (err)
        fn(null, new Input([{ file: '-', stdin: true, error: err }]));
      else
        fn(null, new Input([{ file: '-', stdin: true, data: data }]));
    });
  }
  // from files
  else {
